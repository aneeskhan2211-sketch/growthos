import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { PAGE_META } from "@/config/metaTags";
import { useMetaTags } from "@/hooks/useMetaTags";
import { cn } from "@/lib/utils";
import { Link } from "@tanstack/react-router";
import {
  ArrowLeft,
  CheckCircle,
  ChevronDown,
  ChevronUp,
  Copy,
  ExternalLink,
  Globe,
  Image,
  Lock,
  MessageCircle,
  Search,
  Shield,
  Zap,
} from "lucide-react";
import { useState } from "react";
import { SubscriptionPlan } from "../backend";
import { useSubscription } from "../hooks/useSubscription";
import { useWebsiteHealth } from "../hooks/useWebsiteHealth";

// ─── Helpers ──────────────────────────────────────────────────────────────────

function useCopyToClipboard(): [
  string | null,
  (text: string, id: string) => void,
] {
  const [copiedId, setCopiedId] = useState<string | null>(null);
  function copy(text: string, id: string) {
    navigator.clipboard.writeText(text).then(() => {
      setCopiedId(id);
      setTimeout(() => setCopiedId(null), 2000);
    });
  }
  return [copiedId, copy];
}

function CopyButton({
  text,
  id,
  copiedId,
  onCopy,
  label = "Copy Code",
}: {
  text: string;
  id: string;
  copiedId: string | null;
  onCopy: (text: string, id: string) => void;
  label?: string;
}) {
  const isCopied = copiedId === id;
  return (
    <Button
      size="sm"
      variant={isCopied ? "default" : "outline"}
      onClick={() => onCopy(text, id)}
      className={cn("transition-all", isCopied && "btn-primary-glow")}
      data-ocid={`fix_tools.${id}_copy_button`}
    >
      {isCopied ? (
        <CheckCircle size={14} className="mr-1.5" />
      ) : (
        <Copy size={14} className="mr-1.5" />
      )}
      {isCopied ? "Copied!" : label}
    </Button>
  );
}

function ToolCard({
  title,
  icon,
  badge,
  children,
  defaultOpen = false,
}: {
  title: string;
  icon: React.ReactNode;
  badge?: string;
  children: React.ReactNode;
  defaultOpen?: boolean;
}) {
  const [open, setOpen] = useState(defaultOpen);
  return (
    <div
      className="bg-card border rounded-xl shadow-card overflow-hidden"
      data-ocid={`fix_tools.${title.toLowerCase().replace(/\s+/g, "_")}_card`}
    >
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="w-full flex items-center justify-between p-5 text-left hover:bg-muted/30 transition-colors"
      >
        <div className="flex items-center gap-3">
          <span className="text-primary">{icon}</span>
          <span className="font-semibold text-foreground text-sm">{title}</span>
          {badge && (
            <Badge variant="secondary" className="text-xs">
              {badge}
            </Badge>
          )}
        </div>
        {open ? (
          <ChevronUp size={16} className="text-muted-foreground" />
        ) : (
          <ChevronDown size={16} className="text-muted-foreground" />
        )}
      </button>
      {open && (
        <div className="px-5 pb-5 border-t border-border/50 pt-4">
          {children}
        </div>
      )}
    </div>
  );
}

function PaidGate() {
  return (
    <div
      className="flex flex-col items-center py-8 text-center"
      data-ocid="fix_tools.paywall_state"
    >
      <Lock size={28} className="text-muted-foreground mb-3" />
      <p className="text-sm font-bold text-foreground mb-1">
        Upgrade to Growth Plan to unlock this tool
      </p>
      <p className="text-xs text-muted-foreground mb-4">
        Get all 6 code generators — WhatsApp button, meta tags, schema, CTAs,
        robots.txt, and image guide.
      </p>
      <Button
        size="sm"
        className="btn-primary-glow"
        onClick={() => {
          window.location.href = "/paywall";
        }}
        data-ocid="fix_tools.upgrade_button"
      >
        Unlock Fix Tools · ₹299/month
      </Button>
    </div>
  );
}

// ─── Tool 1: WhatsApp Button Generator ────────────────────────────────────────

function WhatsAppTool({
  copiedId,
  onCopy,
  onTrack,
}: {
  copiedId: string | null;
  onCopy: (t: string, id: string) => void;
  onTrack: (type: string) => void;
}) {
  const [phone, setPhone] = useState("");
  const [message, setMessage] = useState(
    "Hi, I'd like to know more about your services.",
  );

  const number = phone.replace(/\D/g, "");
  const encodedMsg = encodeURIComponent(message);
  const snippet = `<!-- Floating WhatsApp Button -->
<a href="https://wa.me/91${number}?text=${encodedMsg}"
  target="_blank"
  rel="noopener noreferrer"
  style="position:fixed;bottom:20px;right:20px;z-index:9999;background:#25D366;color:#fff;border-radius:50%;width:56px;height:56px;display:flex;align-items:center;justify-content:center;text-decoration:none;box-shadow:0 4px 12px rgba(37,211,102,0.4)"
  aria-label="Chat on WhatsApp">
  <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="#fff">
    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/>
    <path d="M12 0C5.373 0 0 5.373 0 12c0 2.122.553 4.112 1.518 5.843L.057 23.686a.5.5 0 00.612.61l5.919-1.55A11.95 11.95 0 0012 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 22c-1.886 0-3.655-.499-5.192-1.373l-.372-.215-3.852 1.01 1.025-3.748-.237-.388A9.961 9.961 0 012 12C2 6.477 6.477 2 12 2s10 4.477 10 10-4.477 10-10 10z"/>
  </svg>
</a>`;

  return (
    <div className="space-y-4">
      <div className="grid sm:grid-cols-2 gap-3">
        <div>
          <label
            htmlFor="wa-phone"
            className="block text-xs font-semibold text-muted-foreground mb-1.5"
          >
            WhatsApp Number (without country code)
          </label>
          <div className="flex">
            <span className="flex items-center px-3 rounded-l-lg bg-muted border border-border border-r-0 text-sm font-semibold text-foreground">
              +91
            </span>
            <input
              id="wa-phone"
              type="tel"
              value={phone}
              onChange={(e) => setPhone(e.target.value)}
              placeholder="9876543210"
              className="flex-1 h-10 px-3 rounded-r-lg bg-input border border-border text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring"
              data-ocid="fix_tools.whatsapp_number_input"
            />
          </div>
        </div>
        <div>
          <label
            htmlFor="wa-message"
            className="block text-xs font-semibold text-muted-foreground mb-1.5"
          >
            Pre-filled message (optional)
          </label>
          <input
            id="wa-message"
            type="text"
            value={message}
            onChange={(e) => setMessage(e.target.value)}
            placeholder="Hi, I'd like to know more…"
            className="w-full h-10 px-3 rounded-lg bg-input border border-border text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring"
            data-ocid="fix_tools.whatsapp_message_input"
          />
        </div>
      </div>

      {/* Preview */}
      {number && (
        <div className="border border-border rounded-xl p-4 bg-muted/20">
          <p className="text-xs font-semibold text-muted-foreground mb-3">
            Live Preview
          </p>
          <div className="flex justify-end">
            <div className="relative w-16 h-32 bg-foreground/5 rounded-2xl border border-border overflow-hidden flex items-end justify-end p-2">
              <div
                className="w-10 h-10 rounded-full flex items-center justify-center shadow-lg"
                style={{ background: "var(--color-whatsapp, #25D366)" }}
              >
                <MessageCircle size={20} fill="white" stroke="none" />
              </div>
            </div>
          </div>
          <p className="text-xs text-muted-foreground mt-2 text-center">
            Your WhatsApp button will appear in the bottom-right corner of your
            website
          </p>
        </div>
      )}

      <div className="bg-muted/40 rounded-lg p-3 font-mono text-xs text-foreground overflow-auto max-h-40 whitespace-pre-wrap">
        {snippet}
      </div>
      <CopyButton
        text={snippet}
        id="whatsapp"
        copiedId={copiedId}
        onCopy={(t, id) => {
          onCopy(t, id);
          onTrack("whatsapp_button");
        }}
        label="Copy WhatsApp Button Code"
      />

      <div className="bg-primary/5 border border-primary/10 rounded-lg p-3">
        <p className="text-xs font-semibold text-primary mb-1">
          How to add this to your website
        </p>
        <ol className="text-xs text-muted-foreground space-y-1 list-decimal list-inside">
          <li>Copy the code above</li>
          <li>Open your website editor (WordPress, Wix, etc.)</li>
          <li>
            Paste it just before the closing <code>&lt;/body&gt;</code> tag
          </li>
        </ol>
      </div>
    </div>
  );
}

// ─── Tool 2: CTA Generator ────────────────────────────────────────────────────

const CTA_OPTIONS = [
  "Book Free Consultation",
  "Get Price on WhatsApp",
  "Call Now",
  "Schedule a Demo",
  "Claim Free Audit",
  "Send Enquiry",
  "Book Appointment",
  "Chat on WhatsApp",
];

function CTATool({
  copiedId,
  onCopy,
  onTrack,
}: {
  copiedId: string | null;
  onCopy: (t: string, id: string) => void;
  onTrack: (type: string) => void;
}) {
  return (
    <div className="space-y-3">
      <p className="text-xs text-muted-foreground">
        Click any CTA to copy it — then paste it on your website, Google Ad, or
        WhatsApp message.
      </p>
      <div className="grid grid-cols-2 gap-2">
        {CTA_OPTIONS.map((cta) => {
          const ctaId = `cta-${cta.toLowerCase().replace(/\s+/g, "-")}`;
          const isCopied = copiedId === ctaId;
          return (
            <button
              key={cta}
              type="button"
              onClick={() => {
                onCopy(cta, ctaId);
                onTrack("cta");
              }}
              className={cn(
                "flex items-center justify-between px-3 py-2.5 rounded-lg border text-sm font-semibold text-left transition-all",
                isCopied
                  ? "bg-primary/10 border-primary/30 text-primary"
                  : "bg-muted/30 border-border hover:border-primary/40 hover:bg-primary/5 text-foreground",
              )}
              data-ocid={`fix_tools.cta_${cta.toLowerCase().replace(/\s+/g, "_")}`}
            >
              <span>{cta}</span>
              {isCopied ? (
                <CheckCircle size={14} className="text-primary shrink-0" />
              ) : (
                <Copy size={14} className="text-muted-foreground shrink-0" />
              )}
            </button>
          );
        })}
      </div>
    </div>
  );
}

// ─── Tool 3: Meta Tags Generator ─────────────────────────────────────────────

function MetaTagsTool({
  copiedId,
  onCopy,
  onTrack,
}: {
  copiedId: string | null;
  onCopy: (t: string, id: string) => void;
  onTrack: (type: string) => void;
}) {
  const [businessName, setBusinessName] = useState("");
  const [service, setService] = useState("");
  const [city, setCity] = useState("");

  const rawTitle =
    service && city && businessName
      ? `${service} in ${city} | ${businessName}`
      : "";
  const title = rawTitle.length > 60 ? `${rawTitle.slice(0, 57)}…` : rawTitle;
  const description =
    businessName && service && city
      ? `Looking for ${service.toLowerCase()} in ${city}? ${businessName} offers professional services at competitive prices. Contact us today for a free consultation.`
      : "";
  const metaDesc =
    description.length > 160 ? `${description.slice(0, 157)}…` : description;

  const snippet = title
    ? `<title>${title}</title>
<meta name="description" content="${metaDesc}" />`
    : "";

  return (
    <div className="space-y-4">
      <div className="grid sm:grid-cols-3 gap-3">
        {[
          {
            label: "Business Name",
            id: "meta-biz",
            value: businessName,
            setter: setBusinessName,
            placeholder: "e.g. Priya Beauty Salon",
            ocid: "meta_business_input",
          },
          {
            label: "Service / Niche",
            id: "meta-service",
            value: service,
            setter: setService,
            placeholder: "e.g. Haircut & Bridal",
            ocid: "meta_service_input",
          },
          {
            label: "City",
            id: "meta-city",
            value: city,
            setter: setCity,
            placeholder: "e.g. Mumbai",
            ocid: "meta_city_input",
          },
        ].map(({ label, id, value, setter, placeholder, ocid }) => (
          <div key={label}>
            <label
              htmlFor={id}
              className="block text-xs font-semibold text-muted-foreground mb-1.5"
            >
              {label}
            </label>
            <input
              id={id}
              type="text"
              value={value}
              onChange={(e) => setter(e.target.value)}
              placeholder={placeholder}
              className="w-full h-10 px-3 rounded-lg bg-input border border-border text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring"
              data-ocid={`fix_tools.${ocid}`}
            />
          </div>
        ))}
      </div>

      {title && (
        <div className="border border-border rounded-xl p-4 bg-muted/10">
          <p className="text-xs font-semibold text-muted-foreground mb-2">
            Google Search Preview
          </p>
          <p className="text-base text-primary font-medium leading-tight mb-0.5 truncate">
            {title}
          </p>
          <p className="text-xs text-muted-foreground leading-relaxed line-clamp-2">
            {metaDesc}
          </p>
          <div className="flex items-center gap-1 mt-1">
            <span
              className={cn(
                "text-xs font-semibold",
                title.length > 55 ? "score-warning" : "score-success",
              )}
            >
              Title: {title.length} chars{" "}
              {title.length > 55 ? "(a bit long)" : "(good)"}
            </span>
          </div>
        </div>
      )}

      {snippet && (
        <>
          <div className="bg-muted/40 rounded-lg p-3 font-mono text-xs text-foreground overflow-auto max-h-24 whitespace-pre-wrap">
            {snippet}
          </div>
          <CopyButton
            text={snippet}
            id="meta-tags"
            copiedId={copiedId}
            onCopy={(t, id) => {
              onCopy(t, id);
              onTrack("meta_tags");
            }}
            label="Copy Meta Tag Code"
          />
        </>
      )}
    </div>
  );
}

// ─── Tool 4: Schema Markup Generator ─────────────────────────────────────────

function SchemaTool({
  copiedId,
  onCopy,
  onTrack,
}: {
  copiedId: string | null;
  onCopy: (t: string, id: string) => void;
  onTrack: (type: string) => void;
}) {
  const [biz, setBiz] = useState("");
  const [address, setAddress] = useState("");
  const [phone, setPhone] = useState("");
  const [hours, setHours] = useState("Mon-Sat 10:00-20:00");

  const schema = biz
    ? JSON.stringify(
        {
          "@context": "https://schema.org",
          "@type": "LocalBusiness",
          name: biz,
          address: { "@type": "PostalAddress", streetAddress: address },
          telephone: phone ? `+91${phone.replace(/\D/g, "")}` : undefined,
          openingHours: hours || undefined,
        },
        null,
        2,
      )
    : "";

  const snippet = schema
    ? `<script type="application/ld+json">\n${schema}\n</script>`
    : "";

  return (
    <div className="space-y-4">
      <div className="grid sm:grid-cols-2 gap-3">
        {[
          {
            label: "Business Name",
            id: "schema-biz",
            value: biz,
            setter: setBiz,
            placeholder: "Priya Beauty Salon",
            ocid: "schema_biz_input",
          },
          {
            label: "Address",
            id: "schema-addr",
            value: address,
            setter: setAddress,
            placeholder: "123, MG Road, Mumbai",
            ocid: "schema_address_input",
          },
          {
            label: "Phone Number",
            id: "schema-phone",
            value: phone,
            setter: setPhone,
            placeholder: "9876543210",
            ocid: "schema_phone_input",
          },
          {
            label: "Opening Hours",
            id: "schema-hours",
            value: hours,
            setter: setHours,
            placeholder: "Mon-Sat 10:00-20:00",
            ocid: "schema_hours_input",
          },
        ].map(({ label, id, value, setter, placeholder, ocid }) => (
          <div key={label}>
            <label
              htmlFor={id}
              className="block text-xs font-semibold text-muted-foreground mb-1.5"
            >
              {label}
            </label>
            <input
              id={id}
              type="text"
              value={value}
              onChange={(e) => setter(e.target.value)}
              placeholder={placeholder}
              className="w-full h-10 px-3 rounded-lg bg-input border border-border text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring"
              data-ocid={`fix_tools.${ocid}`}
            />
          </div>
        ))}
      </div>

      {snippet && (
        <>
          <div className="bg-muted/40 rounded-lg p-3 font-mono text-xs text-foreground overflow-auto max-h-48 whitespace-pre-wrap">
            {snippet}
          </div>
          <CopyButton
            text={snippet}
            id="schema"
            copiedId={copiedId}
            onCopy={(t, id) => {
              onCopy(t, id);
              onTrack("schema");
            }}
            label="Copy Schema Code"
          />
        </>
      )}

      <div className="bg-primary/5 border border-primary/10 rounded-lg p-3">
        <p className="text-xs font-semibold text-primary mb-1">
          How to add this
        </p>
        <ol className="text-xs text-muted-foreground space-y-1 list-decimal list-inside">
          <li>Copy the code above</li>
          <li>
            Paste it inside the <code>&lt;head&gt;</code> tag of your homepage
          </li>
          <li>
            Test it at{" "}
            <a
              href="https://search.google.com/test/rich-results"
              target="_blank"
              rel="noopener noreferrer"
              className="text-primary underline"
            >
              Google's Rich Results Test
            </a>
          </li>
        </ol>
      </div>
    </div>
  );
}

// ─── Tool 5: Robots.txt Guide ─────────────────────────────────────────────────

function RobotsTool({
  copiedId,
  onCopy,
  onTrack,
}: {
  copiedId: string | null;
  onCopy: (t: string, id: string) => void;
  onTrack: (type: string) => void;
}) {
  const [domain, setDomain] = useState("");
  const d =
    domain.replace(/^https?:\/\//, "").replace(/\/$/, "") ||
    "www.yourwebsite.com";
  const template = `User-agent: *
Allow: /

Sitemap: https://${d}/sitemap.xml`;

  return (
    <div className="space-y-4">
      <div>
        <label
          htmlFor="robots-domain"
          className="block text-xs font-semibold text-muted-foreground mb-1.5"
        >
          Your website domain
        </label>
        <input
          id="robots-domain"
          type="text"
          value={domain}
          onChange={(e) => setDomain(e.target.value)}
          placeholder="www.mysalon.com"
          className="w-full h-10 px-3 rounded-lg bg-input border border-border text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring"
          data-ocid="fix_tools.robots_domain_input"
        />
      </div>
      <div className="bg-muted/40 rounded-lg p-3 font-mono text-xs text-foreground whitespace-pre-wrap">
        {template}
      </div>
      <CopyButton
        text={template}
        id="robots"
        copiedId={copiedId}
        onCopy={(t, id) => {
          onCopy(t, id);
          onTrack("robots_txt");
        }}
        label="Copy robots.txt"
      />

      <div className="bg-primary/5 border border-primary/10 rounded-lg p-3">
        <p className="text-xs font-semibold text-primary mb-1">
          How to add robots.txt to your website
        </p>
        <ol className="text-xs text-muted-foreground space-y-1 list-decimal list-inside">
          <li>
            Copy the text above and save it as a file named{" "}
            <code>robots.txt</code>
          </li>
          <li>
            Upload it to the root folder of your website (same folder as your
            homepage)
          </li>
          <li>
            Test it by visiting <code>https://{d}/robots.txt</code> in your
            browser
          </li>
        </ol>
      </div>
    </div>
  );
}

// ─── Tool 6: Image Optimisation Guide ────────────────────────────────────────

function ImageOptTool({
  rawMetrics,
}: {
  rawMetrics?: {
    imageCount: number;
    imagesMissingAlt: number;
    renderBlockingScripts: number;
  };
}) {
  const TIPS = [
    {
      tip: "Convert images to WebP format",
      detail: "WebP images are 25–35% smaller than JPEG with the same quality.",
      link: "https://squoosh.app",
    },
    {
      tip: "Aim for under 100KB per image",
      detail: "Large images are the #1 reason websites load slowly on mobile.",
    },
    {
      tip: "Add alt text to every image",
      detail:
        "Alt text helps Google understand your images and improves search ranking.",
    },
    {
      tip: "Use lazy loading",
      detail:
        'Add loading="lazy" to image tags so images below the fold don\'t slow your page.',
    },
    {
      tip: "Use a CDN for images",
      detail:
        "A content delivery network serves images from a server near your visitor, making load faster.",
    },
  ];

  return (
    <div className="space-y-4">
      {rawMetrics && (
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-muted/30 rounded-xl p-3 text-center border border-border">
            <p className="text-2xl font-bold font-display text-foreground">
              {rawMetrics.imageCount}
            </p>
            <p className="text-xs text-muted-foreground">Images on page</p>
          </div>
          <div className="bg-destructive/5 rounded-xl p-3 text-center border border-destructive/20">
            <p className="text-2xl font-bold font-display score-critical">
              {rawMetrics.imagesMissingAlt}
            </p>
            <p className="text-xs text-muted-foreground">Missing alt text</p>
          </div>
        </div>
      )}

      <div className="space-y-3">
        {TIPS.map(({ tip, detail, link }) => (
          <div
            key={tip}
            className="flex gap-3 items-start p-3 rounded-lg bg-muted/20 border border-border/50"
          >
            <div className="w-6 h-6 rounded-full bg-primary/10 flex items-center justify-center shrink-0 mt-0.5">
              <Image size={12} className="text-primary" />
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold text-foreground">{tip}</p>
              <p className="text-xs text-muted-foreground mt-0.5">{detail}</p>
              {link && (
                <a
                  href={link}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-xs text-primary flex items-center gap-1 mt-1 hover:underline"
                >
                  Try {new URL(link).hostname} <ExternalLink size={10} />
                </a>
              )}
            </div>
          </div>
        ))}
      </div>
    </div>
  );
}

// ─── Main Page ────────────────────────────────────────────────────────────────

export default function WebsiteHealthFixToolsPage() {
  useMetaTags(PAGE_META["/website-health/fix-tools"]);
  const { latestAudit, trackEvent } = useWebsiteHealth();
  const { data: subscription } = useSubscription();
  const [copiedId, onCopy] = useCopyToClipboard();

  const isPaid =
    subscription?.plan === SubscriptionPlan.growth ||
    subscription?.plan === SubscriptionPlan.pro ||
    subscription?.plan === SubscriptionPlan.agency;

  async function handleTrack(generatorType: string) {
    await trackEvent("fix_clicked", generatorType);
  }

  const toolProps = { copiedId, onCopy, onTrack: handleTrack };

  return (
    <div
      className="max-w-3xl mx-auto px-4 py-6 space-y-6"
      data-ocid="fix_tools.page"
    >
      {/* ── Header ──────────────────────────────────────────────────── */}
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div>
          <Link
            to="/website-health"
            className="flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground mb-2 transition-colors"
            data-ocid="fix_tools.back_button"
          >
            <ArrowLeft size={14} />
            Back to Health Check
          </Link>
          <h1 className="text-2xl font-bold text-foreground font-display">
            Fix Tools
          </h1>
          <p className="text-sm text-muted-foreground mt-0.5">
            Copy-paste code generators to fix common website issues — no
            developer needed.
          </p>
        </div>
        {latestAudit && (
          <div
            className="bg-card border rounded-xl px-4 py-2 flex items-center gap-2 shadow-sm"
            data-ocid="fix_tools.last_scan_badge"
          >
            <Globe size={14} className="text-primary shrink-0" />
            <div className="min-w-0">
              <p className="text-xs text-muted-foreground">
                Based on your last scan
              </p>
              <p className="text-xs font-semibold text-foreground truncate max-w-40">
                {latestAudit.url}
              </p>
            </div>
            <Badge variant="secondary" className="text-xs shrink-0">
              Score: {latestAudit.overallScore}
            </Badge>
          </div>
        )}
      </div>

      {!isPaid && (
        <div
          className="bg-premium-accent-light border border-border rounded-xl p-4 flex flex-col sm:flex-row items-center gap-3"
          data-ocid="fix_tools.upgrade_banner"
        >
          <Lock size={20} className="text-premium-accent shrink-0" />
          <div className="flex-1 text-center sm:text-left">
            <p className="text-sm font-bold text-foreground">
              Fix Tools require Growth Plan
            </p>
            <p className="text-xs text-muted-foreground">
              All 6 code generators are included in Growth (₹299/month) and
              above.
            </p>
          </div>
          <Button
            size="sm"
            className="btn-primary-glow shrink-0"
            onClick={() => {
              window.location.href = "/paywall";
            }}
            data-ocid="fix_tools.top_upgrade_button"
          >
            Upgrade · ₹299/month
          </Button>
        </div>
      )}

      {/* ── Tools ───────────────────────────────────────────────────── */}
      <div className="space-y-4">
        <ToolCard
          title="WhatsApp Button"
          icon={<MessageCircle size={18} />}
          badge="Most Popular"
          defaultOpen={isPaid}
        >
          {isPaid ? <WhatsAppTool {...toolProps} /> : <PaidGate />}
        </ToolCard>

        <ToolCard
          title="CTA Text Generator"
          icon={<Zap size={18} />}
          badge="Quick Win"
        >
          {isPaid ? <CTATool {...toolProps} /> : <PaidGate />}
        </ToolCard>

        <ToolCard title="Meta Title & Description" icon={<Search size={18} />}>
          {isPaid ? <MetaTagsTool {...toolProps} /> : <PaidGate />}
        </ToolCard>

        <ToolCard
          title="Schema Markup (Local Business)"
          icon={<Shield size={18} />}
        >
          {isPaid ? <SchemaTool {...toolProps} /> : <PaidGate />}
        </ToolCard>

        <ToolCard title="Sitemap & robots.txt Guide" icon={<Globe size={18} />}>
          {isPaid ? <RobotsTool {...toolProps} /> : <PaidGate />}
        </ToolCard>

        <ToolCard title="Image Optimisation Guide" icon={<Image size={18} />}>
          {isPaid ? (
            <ImageOptTool rawMetrics={latestAudit?.rawMetrics} />
          ) : (
            <PaidGate />
          )}
        </ToolCard>
      </div>

      <p className="text-xs text-muted-foreground text-center pb-4">
        These are copy-paste tools only. Automatic changes require connecting
        your website/CMS.
      </p>
    </div>
  );
}
