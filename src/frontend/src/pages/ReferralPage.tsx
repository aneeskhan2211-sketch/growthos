...
import type { ReferralRecord, ReferralStats } from "@/backend";
...
import { useMilestones } from "../hooks/useViralLoop";
import type { MilestoneInfo } from "../types/viralLoop";
...
function logShare(channel: string) {
  try {
    if (typeof window !== "undefined" && "analytics" in window) {
      // record analytics event if available
    }
  } catch {
    // silent
  }
...
function StatTile({ label, value, icon, ocid }: StatTileProps) {
  return (
    <Card data-ocid={ocid} className="bg-card border-border text-center">
      <CardContent className="pt-5 pb-4 px-4">
        <div className="flex items-center justify-center w-10 h-10 rounded-xl bg-primary/10 mx-auto mb-3">
          {icon}
        </div>
        <p className="text-xl font-display font-bold text-foreground tabular-nums mb-0.5">
          {value}
        </p>
...
function MiniMilestoneCard({ milestone: m }: { milestone: MilestoneInfo }) {
  const pct = Math.min(
    100,
      ...
      <div className="flex items-center justify-between">
        ...
        {m.status === "locked" && (
          ...
          >
            ...
            <path d="M5 6 L5 4 C5 2.9 5.9 2 7 2 C8.1 2 9 2.9 9 4 L9 6" />
          </svg>
        )}
        {m.status === "unlocked" && (
      ...
      <div>
        <div className="flex items-center justify-between mb-1">
          <span className="text-[10px] text-muted-foreground">
            {m.currentInvites}/{m.requiredInvites} invites
          </span>
      ...
      <CardHeader className="pb-3 flex-row items-start justify-between gap-2">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            Refer friends to unlock powerful features for free
          </p>
        </div>
        <Button
          size="sm"
...
export default function ReferralPage() {
  ...
  const rewardsLabel = stats
    ? `${Number(stats.creditsEarned)} credits + ${Number(stats.trialDaysEarned)} days`
    : "0 credits + 0 days";
        ...
        >
          <Card className="bg-gradient-to-br from-primary/10 via-card to-card border-primary/20 overflow-hidden">
            <CardContent className="pt-8 pb-6 px-6 text-center">
              {/* Trophy icon */}
              <div className="flex items-center justify-center w-16 h-16 rounded-2xl bg-primary/15 mx-auto mb-4 shadow-sm">
                <svg
                  width="28"
                  height="28"
                  viewBox="0 0 28 28"
              ...
              <p className="text-sm text-muted-foreground max-w-xs mx-auto leading-relaxed">
                ...
                <strong className="text-foreground">BOTH</strong> get:
              </p>

              <div className="flex items-center justify-center gap-3 mt-4 flex-wrap">
                <span className="inline-flex items-center gap-1.5 bg-primary/12 text-primary text-xs font-semibold px-3 py-1.5 rounded-full border border-primary/20">
                  <svg
                    width="12"
                    height="12"
                    viewBox="0 0 12 12"
                ...
                <span className="inline-flex items-center gap-1.5 bg-success/12 text-success text-xs font-semibold px-3 py-1.5 rounded-full border border-success/20">
                  <svg
                    width="12"
                    height="12"
                    viewBox="0 0 12 12"
              ...
              {referralsLoading ? (
                ...
                >
                  ...
                  <p className="text-xs text-muted-foreground">
                    Share your link above to invite friends and start earning
                    rewards.
                  </p>
                </div>
              ) : (
                <div className="divide-y divide-border">
                  {referrals.map((ref, idx) => {
                    const dateStr = ref.createdAt
                      ...
                      : "—";
                    const userLabel = ref.referredUserId
                      ? `...${String(ref.referredUserId).slice(-6)}`
                      : "Invited";
